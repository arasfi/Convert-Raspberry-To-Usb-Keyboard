...
class JavascriptKeyEvent {
    constructor(meta_modifier, alt_modifier, shift_modifier, ctrl_modifier, key, key_code) {
        this.meta_modifier = meta_modifier;
        this.alt_modifier = alt_modifier;
        this.shift_modifier = shift_modifier;
        this.ctrl_modifier = ctrl_modifier;
        this.key = key;
...
function convert(js_key_event) {
    let control_chars = 0;

    [js_key_event.ctrl_modifier, js_key_event.shift_modifier, js_key_event.alt_modifier, js_key_event.meta_modifier]
        .forEach((pressed, i) => {
            if (pressed) {
                control_chars |= 1 << i;
            }
    ...
    const key_code = JS_TO_HID_KEYCODES[js_key_event.key_code];

    if (key_code === undefined) {
        throw new UnrecognizedKeyCodeError(`Unrecognized key code ${js_key_event.key} (${js_key_event.key_code})`);
    }

    return [control_chars, key_code];
}

module.exports = {
    JavascriptKeyEvent: JavascriptKeyEvent,
    convert: convert,
};